...
def get_network(image_size, widths, block_depth):
    ...
    m = keras.Model([noisy_images, noise_variances], x, name="residual_unet")
    m.summary()
    return m
...
class DiffusionModel(keras.Model):
    ...
    def plot_images(self, epoch=None, logs=None, num_rows=3, num_cols=6, waiting_time=12000):
        ...
        timer = fig.canvas.new_timer(interval=waiting_time)  # creating a timer object and setting an interval of 3000 milliseconds
    ...
    def plot_images_cv2(self, epoch=None, logs=None, waiting_time=3000):
        generated_image = self.generate(1, plot_diffusion_steps)
        generated_image = np.reshape(generated_image, (image_size, image_size, 3))
        cv2.imshow("image samples", generated_image)
...
model.fit(
    ...
    callbacks=[
        ...
        #keras.callbacks.LambdaCallback(on_epoch_end=model.plot_images_cv2),
...
model.plot_images_cv2(waiting_time=0)
model.plot_images_cv2(waiting_time=0)
model.plot_images_cv2(waiting_time=0)
model.plot_images_cv2(waiting_time=0)
model.plot_images_cv2(waiting_time=0)
