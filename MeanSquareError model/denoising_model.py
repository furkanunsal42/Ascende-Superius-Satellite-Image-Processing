...
denoiser.input_filepath = "newyork_high_noisy_monochrome_images/"
denoiser.target_filepath = "newyork_monochrome_images/"
denoiser.data_amount = 1024
denoiser.slide = 0

#denoiser.train(5, save=True, read=1)

test = cv.resize(cv.cvtColor(cv.imread("newyork_high_noisy_monochrome_images/image_0.png"), cv.COLOR_BGR2GRAY), (IMAGE_SIZE, IMAGE_SIZE))
test = np.reshape(test, (1, IMAGE_SIZE, IMAGE_SIZE, 1))
test = np.asarray(test, np.float32) / 255.0
denoiser.model.load_weights("model0.h5")
